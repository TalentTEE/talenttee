...
export async function approveAgreement(sessionId: string): Promise<{ status: string; txParams?: any }> {
  if (USE_DUMMY) return { status: 'both_approved' };
  return apiFetch(`/negotiation/sessions/${sessionId}/approve`, { method: 'POST' });
}

export async function confirmAgreementTx(sessionId: string, txHash: string): Promise<void> {
  await apiFetch(`/negotiation/sessions/${sessionId}/confirm-tx`, {
    method: 'POST',
