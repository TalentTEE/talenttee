...
| 8 | 매칭 통보 + 양쪽 동의 | MUST | 매칭 결과 양측 통보 → 양쪽 동의 시 협상 세션 생성 |
| 9 | TEE 자율 협상 | MUST | NEAR AI Cloud TEE에서 에이전트 간 N라운드 협상, 실시간 라운드 표시 + 사용자 중간 개입 가능 |
| 10 | 합의 → 온체인 기록 | MUST | 사용자 확인 후 합의 해시 + 조건 요약 + 양측 서명을 NEAR 블록체인에 기록 |
| 11 | 에스크로 예치 + 자동결제 | MUST | 채용담당자가 직접 금액 입력하여 NEAR 예치, 합의 시 Function Call Key로 자동 결제 |
| 12 | 협상 히스토리 열람 | MUST | ECDH 복호화(지갑 자동 처리)로 양측 협상 라운드 전체 열람 |
| 13 | E2E 암호화 | MUST | Ed25519→Curve25519 변환, X25519 ECDH 공유키, XChaCha20-Poly1305 대칭 암호화 |
...
- **설명**: 매칭 결과를 양측에 통보하고, 양쪽이 동의하면 협상 세션을 생성한다
- **사용자**: 구직자, 채용담당자
- **트리거**: 매칭 결과 생성 후
- **입력**: 매칭 결과 + 양측 동의 여부
- **동작**:
  ...
  2. 구직자가 동의 + 채용담당자가 동의
  3. 양쪽 동의 시 NegotiationSession 생성 (state: INITIATED)
  4. ECDH 세션키 유도 (Ed25519→Curve25519 변환, X25519 ECDH, HKDF-SHA256)
- **출력/결과**: NegotiationSession 레코드 (state: INITIATED, sessionKey_nonce)
- **예외 처리**: 한쪽이 거절하면 세션 미생성, 다음 매칭으로 진행
- **수락 기준(AC)**:
  - Given 매칭 결과가 양측에 표시된 상태일 때
  - When 양쪽 모두 동의 버튼을 클릭하면
  - Then 협상 세션이 INITIATED 상태로 생성된다

#### FR-009: TEE 자율 협상 (실시간 + 사용자 개입)
...
#### FR-010: 합의 확인 → 온체인 기록 + 에스크로 결제

- **설명**: 협상 합의 후, 양측 사용자가 합의 내용을 확인하고 승인하면, 합의 해시를 NEAR 블록체인에 기록하고 에스크로 결제를 실행한다
- **사용자**: 구직자, 채용담당자
...
- **동작**:
  ...
  4. Escrow Contract: `release_payment()` 호출 — Function Call Key로 자동 결제 (지갑 팝업 없음)
  5. PaymentRecord에 agreement_hash 기록 (결제 근거)
  6. 양측에 "합의 완료, 면접 프로세스로 진행" 통보
- **출력/결과**: 온체인 AgreementRecord + PaymentRecord + 양측 통보
- **예외 처리**: 한쪽 미승인 시 대기, 에스크로 잔액 부족 시 추가 예치 안내
- **수락 기준(AC)**:
  - Given 협상이 AGREED 상태이고 양측이 합의 내용을 확인한 상태일 때
  - When 양측 모두 승인 버튼을 클릭하면
  - Then 합의 해시가 NEAR 블록체인에 기록되고, 에스크로에서 자동 결제가 실행된다

#### FR-011: 에스크로 예치

- **설명**: 채용담당자가 원하는 금액을 직접 입력하여 에스크로 컨트랙트에 NEAR를 예치하고, 에이전트에게 Function Call Access Key를 부여한다
- **사용자**: 채용담당자
...
- **수락 기준(AC)**:
  ...
  - Then 에스크로 잔액이 반영되고, 에이전트가 release_payment를 호출할 수 있게 된다
...
| NFR-9 | 마감 | PoC 마감: 2026-04-15 (4일) | 해커톤 제출 기한 |
...
| 2 | Meta Transaction(NEP-366) 릴레이어 범위 | "팀 논의 필요" | PoC에서는 에스크로 TX만 적용, 나머지는 이후 확장 | 4일 내 구현 가능 범위 |
...
| 9 | 프론트엔드 구현 범위 | "팀 논의 필요" | 핵심 플로우 중심 (로그인→협상→결제 데모 플로우) | PoC 4일 내 구현 가능 범위 |
...
| 4. Defensive Check | 3 | MAX_ROUNDS 미확정, PoC 마감 4/15(4일), Meta TX 미확정 |
...
*작성일: 2026-04-11*
*상태: DRAFT — 요청자 합의 대기*
*기반 문서: docs/superpowers/specs/2026-04-10-career-agent-prd.md*
